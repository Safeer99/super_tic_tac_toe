...
const startpage = document.getElementById("landing-page");
...
// player properties
const Players = [
  { symbol: "O", color: "rgb(0, 106, 255)", color_name: "blue" },
  { symbol: "X", color: "red", color_name: "red" },
];

// turn management
let current_turn = Math.round(Math.random());
setTurnText(
  getTurnText(Players[current_turn].symbol, Players[current_turn].color)
);
  ...
  if (!win) {
    if (checkPlaceable(cell, field)) {
      ...
      const checked_cells = Array.from(field.children).map((cell) =>
        cell.classList.contains(Players[current_turn].color_name)
      );
      if (
      ...
      const checked_fields = Array.from(Game.children).map((field) =>
        field.classList.contains(Players[current_turn].color_name)
      );
      if (
    ...
    if (win) {
      setTurnText(
        `🏆 <span style="color:${Players[opposite(current_turn)].color};">${
          Players[opposite(current_turn)].symbol
        }</span> 🏆`
      );
      removeTemporaryClasses();
      confetti();
